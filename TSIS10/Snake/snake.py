...
# Function to add new user to database with initial states

def insert_user_with_initial_states(username, level=1, score=0):
    cur = conn.cursor()
...
def insert_user_score(user_id, level, score, username):
    cur = conn.cursor()
    cur.execute("INSERT INTO user_score (user_id, level, score) VALUES (%s, %s, %s)", (user_id, level, score))
    conn.commit()
    print(get_user_data(username))

def update_user_data(username, new_level, new_score):
    cur = conn.cursor()
    # Update the user's level and score in the user_score table
    cur.execute("""
        UPDATE user_score 
        SET level = %s, score = %s 
        WHERE user_id = (
...
def handle_events(username, level, score):
    ...
    if keys[pygame.K_p]:
        insert_user_score(get_user_id(username), level, score, username)
...
#     cur = conn.cursor()
#     cur.execute("SELECT level FROM \"User\" WHERE username = %s", (username,))
#     return cur.fetchone()[0]
...
def get_user_data(username):
    cur = conn.cursor()
    # Joining "User" and "user_score" tables to retrieve level and score data
    cur.execute("""
        SELECT u.username, us.level, us.score 
        FROM "User" u 
        JOIN user_score us ON u.user_id = us.user_id 
        WHERE u.username = %s
    """, (username,))
...
def game_loop(username):
    game_over = False
    game_close = False
    username, level, score = get_user_data(username)
    ...
    while not game_over:
        ...
        handle_events(username, level, score)
        ...
        if snake.positions[0] == food.position:
            if generate_weighted_food() == 'normal':
                snake.eat()
                score += 1
                update_user_data(username, level, score)
            else:
...
def main():
    ...
    if user_exists(username):
        ...
        insert_user_with_initial_states(username, level=1, score=0)
